...
const DriverCard = ({ item, selected, setSelected }: DriverCardProps) => {
        return (
                <TouchableOpacity onPress={setSelected} className={`${selected === item.id ? 'bg-general-600' : 'bg-white'} flex flex-row items-center justify-between py-5 px-3 rounded-xl`}>
                        ...
                        <View className="flex-1 flex flex-col items-start justify-center mx-3">
                                ...
                                <View className="flex flex-row items-center justify-start">
                                        ...
                                        <Text className="text-sm font-JakartaRegular text-general-800">{formatTime(parseInt(`${item.time}`))}</Text>
